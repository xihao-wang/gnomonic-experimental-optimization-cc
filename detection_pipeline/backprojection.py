...
def backproject_bbox(bbox: Dict, metadata: Dict,
    ...
    # Forward any private/flag keys (underscore-prefixed) so downstream
    # filters can act on them. Example: '_flagged_border_side' from the
    # redundant-bbox filter must survive to fisheye for the confirmation pass.
    for key, value in bbox.items():
        if key.startswith('_'):
            radial_bbox[key] = value
