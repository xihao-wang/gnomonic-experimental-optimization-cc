"""
Backprojection visual validation pipeline.

This module orchestrates visual validation testing of backprojected bounding boxes
from multiple projection configurations. It generates side-by-side visualizations
for manual inspection to verify backprojection accuracy.

PURPOSE: Visual validation only - NOT for computing evaluation metrics.
Future evaluation metrics (precision, recall, mAP) will be in separate modules.

Author: Yassir Zardoua
Date: 2025-12-04
Updated: 2026-01-22 (Renamed to clarify separation from evaluation metrics)
"""
...
from evaluation.lib.dataset_registry import get_dataset_structure, list_available_datasets
from evaluation.lib.output_manager import OutputPathManager
from evaluation.lib.visualization import (
    visualize_gt_and_predictions,
    visualize_composite_detections,
...
class BackprojectionVisualValidator:
    """Visual validator for backprojected bounding boxes from different projection configurations."""
    ...
    def run_validation(self, dataset_names: List[str]):
        """
        Run visual validation on specified datasets.

        Generates side-by-side visualizations for manual inspection of
        backprojection accuracy. Does NOT compute evaluation metrics.
        ...
        # Run visual validation for each configuration
    ...
    def _process_image(
        ...
        # Generate side-by-side visualization (fisheye + composite)
        self._save_side_by_side_visualization(
            image, composite, gt_annotations, predictions, config_id, dataset_name,
            relative_path, dataset_structure["fisheye_center"]
        )
    ...
    def _save_side_by_side_visualization(
        self,
        ...
        """Save side-by-side visualization (fisheye left + composite right)."""
        # Convert relative path from .json to .jpg
        vis_relative_path = relative_path.replace('.json', '.jpg')

        # Load already-rendered images from all-images/ folders
        # These images already have bboxes drawn on them
        fisheye_rendered_path = self.output_manager.get_all_images_fisheye_path(
            config_id, dataset_name, vis_relative_path
        )
        composite_rendered_path = self.output_manager.get_all_images_composite_path(
            config_id, dataset_name, vis_relative_path
        )

        # Read the already-rendered images
        fisheye_rendered = cv2.imread(str(fisheye_rendered_path))
        composite_rendered = cv2.imread(str(composite_rendered_path))

        if fisheye_rendered is None or composite_rendered is None:
            if self.verbose:
                print(f"WARNING: Could not load rendered images for {vis_relative_path}")
            return

        # Create side-by-side visualization by simply stacking the two images
        side_by_side_image = create_side_by_side_visualization(
            fisheye_rendered,
            composite_rendered
        )

        # Save to flattened structure
        output_path = self.output_manager.get_all_images_side_by_side_path(
            config_id, dataset_name, vis_relative_path
        )
        self.output_manager.ensure_directories_exist(output_path)
        cv2.imwrite(str(output_path), side_by_side_image)
