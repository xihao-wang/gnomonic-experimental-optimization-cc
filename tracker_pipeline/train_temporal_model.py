#!/usr/bin/env python3
"""Train the StrongSORT temporal attention scorer inside this project."""
...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Load one npz shard at a time during training/evaluation. "
            "Use this for many large shard files to avoid concatenating all arrays in RAM."
        ),
    )
...
def make_train_loader(dataset, batch_size, seed, no_group_batches):
    if not no_group_batches and hasattr(dataset, "group_id"):
        sampler = SameDetectionBatchSampler(
            dataset.group_id,
...
def scan_npz_stats(npz_paths, history_len, long_history_len):
    """Read only per-file metadata needed before streaming training.

    Each file is opened independently and released immediately, so memory usage
    is bounded by the largest shard instead of the sum of all shards.
    """
    if not npz_paths:
        raise ValueError("npz_paths must not be empty")
    total = 0
    ...
    for npz_path in npz_paths:
        with np.load(npz_path) as data:
            ...
            det_feat = data["det_feat"]
            short_hist_feat = data["short_hist_feat"]
            long_hist_feat = data["long_hist_feat"]
            label = data["label"]
            if feature_dim is None:
                feature_dim = det_feat.shape[1]
            elif det_feat.shape[1] != feature_dim:
            ...
            if short_hist_feat.shape[1] != history_len:
                raise ValueError(
                    f"{npz_path} short_hist_feat has length {short_hist_feat.shape[1]}, expected {history_len}"
                )
            if long_hist_feat.shape[1] != long_history_len:
                raise ValueError(
                    f"{npz_path} long_hist_feat has length {long_hist_feat.shape[1]}, expected {long_history_len}"
                )
            total += int(label.shape[0])
            num_pos += float(label.astype(np.float32).sum())
    return feature_dim, total, num_pos


def run_train_loader(
    model,
    loader,
    criterion,
    optimizer,
    ...
    for det, hist, long_hist, short_len, long_len, label, group_id, _track_id, track_group_id in loader:
    ...
    return {
        "loss": running_loss,
        "bce_loss": running_bce_loss,
        "rank_loss": running_rank_loss,
        "track_rank_loss": running_track_rank_loss,
        "total": total,
        "ranking_batches": ranking_batches,
    }


def evaluate_npz_paths(model, npz_paths, criterion, args):
    total_loss = 0.0
    total = 0
    correct = 0
    for npz_path in npz_paths:
...
def main():
    ...
    if use_explicit_split:
        ...
        if args.stream_npz:
            feature_dim, train_total, train_pos = scan_npz_stats(
                args.train_pair_npz,
                history_len=args.history_len,
                long_history_len=args.long_history_len,
            )
            val_feature_dim, val_total, _val_pos = scan_npz_stats(
                args.val_pair_npz,
                history_len=args.history_len,
                long_history_len=args.long_history_len,
            )
            ...
            if val_feature_dim != feature_dim:
                raise ValueError(
                    f"Feature dimension mismatch between train ({feature_dim}) and val ({val_feature_dim}) datasets"
                )
            labels = None
            num_pos = float(train_pos)
            num_neg = float(train_total - train_pos)
            train_set = None
            val_set = None
        else:
        ...
        if args.stream_npz:
            raise ValueError("--stream_npz requires explicit --train_pair_npz and --val_pair_npz inputs")
    ...
    if args.stream_npz:
        ...
        train_loader = make_train_loader(
            ...
            seed=args.seed,
            no_group_batches=args.no_group_batches,
        )
    ...
    print(
        ...
        f"group_batches={not args.no_group_batches}, "
        f"stream_npz={args.stream_npz})"
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.stream_npz:
            ...
            train_paths = list(args.train_pair_npz)
            ...
            for npz_idx, npz_path in enumerate(train_paths, start=1):
                shard_set = TemporalPairDataset(
                    npz_path,
                    history_len=args.history_len,
                    long_history_len=args.long_history_len,
                )
                shard_loader = make_train_loader(
                    shard_set,
                    batch_size=args.batch_size,
                    seed=args.seed + epoch + npz_idx,
                    no_group_batches=args.no_group_batches,
                )
                shard_stats = run_train_loader(
                    model,
                    shard_loader,
                    criterion,
                    optimizer,
            ...
            running_bce_loss = stats["bce_loss"]
            running_rank_loss = stats["rank_loss"]
            running_track_rank_loss = stats["track_rank_loss"]
            total = stats["total"]
            ranking_batches = stats["ranking_batches"]
        else:
            stats = run_train_loader(
                model,
                train_loader,
                criterion,
                optimizer,
        ...
        if args.stream_npz:
            val_loss, val_acc = evaluate_npz_paths(model, args.val_pair_npz, criterion, args)
        else:
